...
# Python 2.7 Doesn't support dataclasses, so we have to do this the old-fashioned way:
class AtlasItem:
...
class Atlas:
    ...
    def savePreset(self):
        ...
        file = cmds.fileDialog(m = 1, dm = dir + '*.atl')
    ...
    def loadPreset(self):
        ...
        file = cmds.fileDialog(m = 0, dm = dir + '*.atl')
...
class EasyAtlas():
    ...
    def __init__(self):
        ...
        self.windowUI = qt_utils.loadQtWindow(self._uiFile, self.windowName)
        ...
        cmds.dockControl(
            self.dockName,
            allowedArea = ["right", "left"], area = "right",
            content = self.windowName,
            visible = True
        )
    ...
    def about(self):
        cmds.confirmDialog(
            t = "About",
            message = "Easy Atlas was made by Seigi Sato.\n"
                      "More info at www.seigisato.com.",
            button = ["Ok"]
        )
    ...
    def addEAtoShelf(self):
        shelf = mel.eval(""
                         ...
                         "")
        cmds.shelfButton(
            rpt = True, i1 = self._easyAtlasIcon, ann = "Easy Atlas",
            command = "import easy_atlas\neasy_atlas.launch()", p = shelf
        )
    ...
    def pickPhotoshopPath(self):
        ...
        photoshopPath = cmds.fileDialog(m = 0, dm = '%s/*.exe' % folder)
    ...
    def setAtlasIdToMesh(self, mesh):
        ...
        for k in table.selectedItems():
            ...
            if selectedCoord in allTakenCoords:
                cmds.confirmDialog(
                    t = diaWarning,
                    message = "Cannot overlap Atlas region.",
                    button = ["ok"]
                )
                return
    ...
    def assignTextureToMesh(self):
        ...
        file = cmds.fileDialog(m = 0, dm = dir + '/*.*')
        if file:
            tableMeshes = qt_utils.getControl(self._meshTable)
    ...
    def resizeAtlasTable(self, resetItems=True):
        ...
        Clears id, color and coords info from meshes.
        """
        table = qt_utils.getControl(self._atlasTable)
    ...
    def addMesh(self):
        ...
        if not meshes:
            cmds.confirmDialog(
                t = diaWarning,
                message = "At least one mesh must be selected for adding.",
                button = ["ok"]
            )
            return
        ...
        for k in meshes:
            if not self.AtlasInfo.getAtlasMeshByName(k):
                texture = ""
                try:
                    rel = cmds.listRelatives(k)
                    sg = cmds.listConnections(rel, type = "shadingEngine")
                    materials = cmds.listConnections(sg[0] + ".surfaceShader")
                    files = cmds.listConnections(materials[0], type = "file")
                    texture = cmds.getAttr(files[0] + ".fileTextureName")
                except:
                    pass
    ...
    def addMeshFromViewportSelection(self):
        ...
        if not meshes:
            cmds.confirmDialog(
                t = diaWarning,
                message = "A mesh must be selected for adding.",
                button = ["ok"]
            )
            return
        ...
        if len(meshes) > 1:
            cmds.confirmDialog(
                t = diaWarning,
                message = "Please select only one mesh at a time.",
                button = ["ok"]
            )
            return
    ...
    def pickOutputTexture(self):
        ...
        file = cmds.fileDialog(m = 1, dm = dir + '/*.*')
        if file:
    ...
    def makeAtlas(self):
        ...
        if not photoshopPath:
            setUpPS = cmds.confirmDialog(
                t = diaWarning,
                message = "Photoshop path missing. Do you want to pick a Photoshop path now?",
            ...
            if setUpPS == "No":
                ...
                photoshopPath = cmds.fileDialog(m = 0, dm = 'c:/*.exe')
                if os.path.exists(photoshopPath):
        ...
        if os.path.splitext(txtFinalFilename)[1] not in SupportedFileExtensions:
            cmds.confirmDialog(
                t = diaWarning,
                message = "Output file type not supported by Easy Atlas.\n "
                          "Supported types are {}".format(SupportedFileExtensions),
                button = ["ok"]
            )
            return
        ...
        for mesh in self.AtlasInfo.listOfAtlasMeshes:
            ...
            if mesh.id != -1:
                ...
                if not os.path.exists(mesh.texture):
                    cmds.confirmDialog(
                        t = diaWarning,
                        message = "Input texture doesn't exist: \n%s." % mesh.texture,
                        button = ["ok"]
                    )
                    return
                ...
                try:
                    cmds.ls(mesh.meshName)
                except:
                    cmds.confirmDialog(
                        t = diaWarning,
                        message = "Invalid mesh name: \n%s." % mesh.meshName,
                        button = ["ok"]
                    )
                    return

                if not cmds.ls(mesh.meshName):
                    cmds.confirmDialog(
                        t = diaWarning,
                        message = "Mesh doesn't exist: \n%s." % mesh.meshName,
                        button = ["ok"]
                    )
                    return
        ...
        if not atlasItems:
            cmds.confirmDialog(
                t = diaWarning,
                message = "No item has been assigned to the Atlas.",
                button = ["ok"]
            )
            return

        texture_atlas.createAtlas(
            atlasItems,
            txtFinalFilename,
        ...
        cmds.setAttr(file_node + '.fileTextureName', txtFinalFilename, type = 'string')
...
if bool(mel.eval('getenv "EASY_DEBUG_MODE"')):
    launch()
